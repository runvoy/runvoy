...
- **Local iteration**: `just run-local` launches the local HTTP server; `just dev-server` wraps it with `reflex` for hot reload. You can use the CLI for full API testing once the server is running.
