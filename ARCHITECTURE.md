...
**Location:** `lambda/orchestrator/main.go`
**Language:** Go
...
**Key Functions:**
- `handler()` - Main Lambda entry point, routes to action handlers
- `handleExec()` - Creates and starts ECS tasks
- `handleStatus()` - Queries ECS task status
- `handleLogs()` - Fetches CloudWatch logs
- `buildShellCommand()` - Constructs git clone + command execution script
- `buildDirectCommand()` - Constructs direct command execution (--skip-git mode)
...
4. Lambda (lambda/orchestrator/main.go:66):
   - Authenticates API key via bcrypt
   - Validates request (repo, command required)
   - Constructs shell script (lambda/orchestrator/main.go:372):
...
2. Lambda (lambda/orchestrator/main.go:156):
   - Skips git credential setup
   - Constructs simpler script (lambda/orchestrator/main.go:326):
...
**Validation:** Bcrypt comparison in Lambda (lambda/orchestrator/main.go:112)
...
- Deleted before container exits (lambda/orchestrator/main.go:469)
...
**Current Implementation:** Dynamic bash script construction in Lambda (lambda/orchestrator/main.go:372)
...
**Security:** Proper shell escaping via `shellEscape()` function (lambda/orchestrator/main.go:316)
...
**Generated by:** `buildShellCommand()` (lambda/orchestrator/main.go:372)
...
**Generated by:** `buildDirectCommand()` (lambda/orchestrator/main.go:326)
...
├── lambda/
│   └── orchestrator/
│       └── main.go            # Lambda handler (lambda/orchestrator/main.go:66)
├── pkg/
│   └── api/
